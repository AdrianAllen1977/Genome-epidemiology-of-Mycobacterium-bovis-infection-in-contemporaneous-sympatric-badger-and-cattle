### Analysing BASTA log files - by Joseph Crispell ### 
#### Preparation ####
...
# Set the path
path <- file.path("~", "Desktop", "TVR_manuscript_Sept20", "Random_subsets_BASTA", "Random_2014_2017_equal_numbers", "Randomisation_1")

# Set the date
date <- format(Sys.Date(), "%d-%m-%y")

# Note the states/demes considered - ALPHABETICAL ORDER
demes <- c("Badger", "Bovine")
...
plotSummaryOfLineageTransitionRates(migrationRateEstimates, demeNames=c("Badgers", "Bovine"))

summariseLineageTransitionRates(migrationRateEstimates)

# Summarise the transition count distributions
plotSummaryOfTransitionCounts(transitionCounts)

summariseTransitionCountTables(transitionCounts)
